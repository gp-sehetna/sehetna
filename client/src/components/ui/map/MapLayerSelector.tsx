...
const MapLayerSelector = ({
    ...
    return (
        <Card
            className={cn(
                className,
                "glassy flex w-full flex-col gap-0.5 bg-neutral-100/40 transition-all duration-300 ease-in-out",
                !isOpen && "pointer-events-none transform border-0 opacity-0"
            )}
        >
            {HEALTH_OUTCOMES_WITH_HYPHEN.map((outcome) => (
                ...
                >
                    ...
                    {outcome === healthOutcome && <Check className="ml-2 h-3.5 w-3.5 shrink-0" />}
                </Button>
            ))}
        </Card>
    )
