...
from src.schema import PredictionRequest, PredictionResult, WeeklyEnvironmentData
...
class Predictor:
    @classmethod
    def get_df(cls, req: PredictionRequest, latitude: float, longitude: float) -> pd.DataFrame:
        country_map, uhc_df, food_access_df, food_stability_df = loader.load_indicators(settings)
    ...
    @classmethod
    def simulate(cls, req: PredictionRequest, latitude: float, longitude: float) -> PredictionResult:
        df = cls.get_df(req, latitude, longitude)
        df_processed = loader.pipeline.transform(df)
        X_pred = df_processed[loader.features]

        predictions = np.mean([model.predict(X_pred) for model in loader.model], axis=0)
