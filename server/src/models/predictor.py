...
class Predictor:
    ...
    def predict(user_input: HealthDataInput) -> Dict:
        input_dict = user_input.__dict__()
        ...
        for target in model_loader.targets:
            df_complete[target] = 0.0

        # Transform
        df_processed = model_loader.pipeline.transform(df_complete)
        
        logger.info(f"After pipeline: {df_processed.shape}")
        
        # Get feature names expected by model (from first estimator)
        try:
            expected_features = model_loader.model.estimators_[0].feature_name_
        except:
        ...
        predictions = model_loader.model.predict(X_pred)
